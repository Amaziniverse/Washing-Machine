...
unsigned int happy_birthday1[] =
{
	 C4,C4,D4,C4,F4,E4,C4
};

unsigned int happy_birthday2[] =
{
...
void buzzer_start(void)
{
	...
	for (int i=0; i < 7; i++)
	{
		__HAL_TIM_SET_AUTORELOAD(&htim5, divide_freq / happy_birthday1[i]);
		__HAL_TIM_SET_COMPARE(&htim5, TIM_CHANNEL_4, divide_freq / happy_birthday1[i] / 2);
		HAL_Delay(300*duration[i]);
		noTone();
	}
}
...
void buzzer_end(void)
{
	...
	for (int i=0; i < 7; i++)
	{
		__HAL_TIM_SET_AUTORELOAD(&htim5, divide_freq / happy_birthday2[i]);
		__HAL_TIM_SET_COMPARE(&htim5, TIM_CHANNEL_4, divide_freq / happy_birthday2[i] / 2);
		HAL_Delay(300*duration[i]);
		noTone();
	}
}
...
void buzzer_alert(void)
{
	...
	for (int i=0; i < 3; i++)
	{
		__HAL_TIM_SET_AUTORELOAD(&htim5, divide_freq / alert[i]);
		__HAL_TIM_SET_COMPARE(&htim5, TIM_CHANNEL_4, divide_freq / alert[i] / 2);
		HAL_Delay(300*duration[i]);
		noTone();
	}
}
